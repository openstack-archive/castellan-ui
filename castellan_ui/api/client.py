...
from castellanclient.v1 import client as castellan_client
...
def apiclient(request):
    ...
    LOG.debug('castellanclient connection created using the token "%s" and url'
              '"%s"' % (request.user.token.id, api_url))
    c = castellan_client.Client(
