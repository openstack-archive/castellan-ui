...
(function() {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.key_manager.symmetric_keys.update.service
   * @description Service for the symmetric_key update modal
   */
  angular
    .module('horizon.dashboard.key_manager.symmetric_keys')
    .factory('horizon.dashboard.key_manager.symmetric_keys.update.service', updateService);
  ...
  function updateService(
    ...
    var message = {
      success: gettext('SymmetricKey %s was successfully updated.')
    };
    ...
    function perform(selected, newScope) {
      ...
      title = gettext("Update SymmetricKey");
      ...
      // load current data
      id = selected.id;
      var deferred = api.getSymmetricKey(id);
    ...
    function allowed() {
      ...
      //return policy.ifAllowed({ rules: [['symmetric_key', 'update_symmetric_key']] });
    }

    function submit() {
      model.cleanProperties();
      return api.updateSymmetricKey(id, model.spec).then(success);
    }
