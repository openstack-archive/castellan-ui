...
class ImportX509Certificate(forms.SelfHandlingForm):
    ...
    cert_file = forms.FileField(
        label=_("Choose file"),
        help_text=_("A PEM encoded certificate file to upload. PEM encoded "
                    "certificates start with -----BEGIN CERTIFICATE-----"),
        widget=forms.FileInput(
            attrs={'class': 'switched', 'data-switch-on': 'source',
                   'data-source-file': _('PEM encoded Certificate File')}),
        required=False)
    direct_input = forms.CharField(
        label=_('PEM encoded Certificate'),
        help_text=_("PEM encoded contents of the certificate. PEM encoded "
                    "certificates start with -----BEGIN CERTIFICATE-----"),
        widget=forms.widgets.Textarea(
            attrs={'class': 'switched', 'data-switch-on': 'source',
                   'data-source-raw': _('PEM encoded Certificate')}),
        required=False)
    ...
    def clean(self):
        ...
        try:
            ...
            msg = _('There was a problem loading the certificate: %s.') % e
            raise forms.ValidationError(msg)

        data['cert_data'] = base64.b64encode(cert_der)
    ...
    def handle(self, request, data):
        try:
            ...
            cert_pem = NEW_LINES.sub("", cert_pem)
